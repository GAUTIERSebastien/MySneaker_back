...
const middlewareAuth = {
  ...
  isLogged(req, res, next) {
    ...
    if (authHeader) {
      ...
      try {
        // je décode le token en fonction avec la clé
        // secrete je stock tous et passe au middlware suivant
        const decoded = jwt.verify(token, process.env.SECRET_JWT);
        req.user = decoded;
        next();
      } catch (err) {
        // si le token a été modifier ou corrumpu alors
        // je renvoie une erreur et la requète s'arrete la
  ...
  isAdmin(req, res, next) {
    // une fois que le token jwt est décodé je récupère l'id
    // de l'user et je vérifie que celui-ci est bien 1
